...
int main(int argc, char **argv) {
  ...
  if (!fin) {
    //        cout<<"please generate the associate file called
    //        associate.txt!"<<endl;
  ...
  string log_file(
      logfilepath + "slamlog" + '-' + int2string(now->tm_year + 1900) + '-' +
      int2string(now->tm_mon + 1) + '-' + int2string(now->tm_mday) + '-' +
      int2string(now->tm_hour) + '-' + int2string(now->tm_min) + '-' +
  ...
  for (int i = 0; i < rgb_files.size(); i++) {
    ...
    //        cv::rectangle(traj, cv::Point(10, 30), cv::Point(580, 60),
    ...
    cv::Affine3d M(cv::Affine3d::Mat3(
                   ...
                   cv::Affine3d::Vec3(Twc.translation()(0, 0),
                                      Twc.translation()(1, 0),
                                      Twc.translation()(2, 0)));
    ...
    for (auto &pt : vo->map_->map_points_) {
      ...
      cv::circle(img_show, cv::Point2f(pixel(0, 0), pixel(1, 0)), 1,
