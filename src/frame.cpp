...
Frame::Frame(long id, double time_stamp, SE3 T_c_w, Camera::Ptr camera,
    ...
    : id_(id), time_stamp_(time_stamp), T_c_w_(T_c_w), camera_(camera),
      color_(color), /*depth_(depth), */ is_key_frame_(false) {}
...
void Frame::insertFramePoint(MapPoint::Ptr map_point,
                             cv::Point2f point_on_frame) {
  if (matched_map_points_.find(map_point->id_) == matched_map_points_.end()) {
    matched_map_points_.insert(make_pair(map_point->id_, map_point));
    matched_map_points_position_.insert(
        make_pair(map_point->id_, point_on_frame));
  } else {
    matched_map_points_[map_point->id_] = map_point;
    matched_map_points_position_[map_point->id_] = point_on_frame;
  }
}
}
