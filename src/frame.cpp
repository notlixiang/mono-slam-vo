...
bool Frame::isDepthPositive ( const Vector3d& pt_world )
{
    Vector3d p_cam = camera_->world2camera( pt_world, T_c_w_ );
    // cout<<"P_cam = "<<p_cam.transpose()<<endl;
    if ( p_cam(2,0)<-0.001 ) return false;
    return true;
}

void Frame::insertFramePoint ( MapPoint::Ptr map_point,cv::Point2f point_on_frame)
{
    if ( matched_map_points_.find(map_point->id_) == matched_map_points_.end() )
    {
        ...
        matched_map_points_position_.insert( make_pair(map_point->id_, point_on_frame) );
    }
    else
    {
